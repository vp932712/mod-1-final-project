...
CLIENT_SECRETS_PATH = './mod-1-final-project/app/models/client_secret.json'
...
def search_list_by_keyword(service, part, params)
  ...
  no_playlists.each do |item|
    puts ("#{item.fetch("snippet").fetch("title")}. " +
        "Its url is https://www.youtube.com/watch?v=#{item.fetch("id").fetch("videoId")}")
  end
end
